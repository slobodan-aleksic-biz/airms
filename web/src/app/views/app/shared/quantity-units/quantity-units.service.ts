...
export class QuantityUnitsService {
  ...
  feedQuantityUnitsDropDown(): Promise<Array<[]>> {
    let url = '/api/' + this.url;
    return this.feedFunc('get', url);
  }

  feedQuantityUnitsDropDownIsConsumableOrNot(isConsumable: boolean): Promise<Array<[]>> {
    let url = '/api/' + this.url + "/" + (isConsumable ? 1 : 0) + "/is-consumable";
    return this.feedFunc('get', url);
  }
